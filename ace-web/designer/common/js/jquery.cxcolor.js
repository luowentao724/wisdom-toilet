...
(function(factory){
	...
	$.cxColor = function(){
		...
		colorPanel.build = function(){
			var _this = this;
			$("div.cxcolor[for='"+_this.dom.el[0].id+"']").remove();
			$("div.cxcolor_lock[for='"+_this.dom.el[0].id+"']").remove();
			var _colorHex = ['00','33','66','99','cc','ff'];
			var _spcolorHex = ['ff0000','00ff00','0000ff','ffff00','00ffff','ff00ff'];
			var _html = '';
			_html = '<div class="panel_hd"><a class="reset" href="javascript://" rel="reset">默认颜色</a>'
				+'<a class="custom" href="javascript://" rel="custom">自定义</a>'
				+'<a class="clear" href="javascript://" rel="clear">清除</a></div>';
			_this.dom.colorPane = $('<div for="'+_this.dom.el[0].id+'" class="cxcolor"></div>').appendTo('body').html(_html);
			
			_html = '<input type="text" rel="custominput"  placeholder="RGB颜色代码如：#ffffff"/><button type="button">确定</button>';
			_this.dom.colorCustom = $('<div></div>', {'class':'custom-input'}).html(_html).appendTo(_this.dom.colorPane);
			...
			_this.dom.lockBackground = $('<div for="'+_this.dom.el[0].id+'" class="cxcolor_lock"></div>').appendTo('body');
			...
			//点击确定设置自定义颜色
			_this.dom.colorCustom.on('click', 'button', function(){
				var customColor = _this.dom.colorCustom.find("input").val();
				_this.dom.colorCustom.hide();
				_this.change(customColor);
			});
	
			// 第一次初始化
			_this.initColor(_this.settings.color);
		};
		...
		// 更改颜色函数
		colorPanel.initColor = function(c){
			this.colorNow = c;
		...
		colorPanel.change = function(c){
			...
			this.dom.colorCustom.find("input").val(c);
			this.hide();
		};
		...
		// 自定义颜色
		colorPanel.custom = function(){
			this.dom.colorCustom.toggle();
			this.dom.colorCustom.find("input").val(this.colorNow);
		};
